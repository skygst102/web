	...
	Sv.point=function(_this){
		...
		if(typeof _this[0]=='object'&&_this!='undefined'){
			return new function() {
				...
				this.tpl=/{{/.test(_thisEl.el[0].Sv_tpl);
...
Sv.Extend({
	...
	enescape:function(str) {
		return String(str).replace(/\$amp;/g,'&').replace(/&lt;/g,'<')
...
Sv.Model('template',function() {
	...
	this.tpl=function() { 
		...
		Sv.each(flagNodes,function(i){
			if (flagNodes[i].nodeName.toLowerCase()!='tpl') {
				var sv_tpl=Sv.escape(flagNodes[i].innerHTML.replace(/\{\{/g,'[[').replace(/\}\}/g,']]'));
				flagNodes[i].setAttribute('Sv_tpl',sv_tpl);
			}	
		});
		...
		Sv.each(newFlag,function(i){
			this[i].Sv_tpl=this[i].getAttribute('Sv_tpl').replace(/\[\[/g,'{{').replace(/\]\]/g,'}}');
			this[i].removeAttribute('Sv_tpl')
		});
	...
	this.view=function(el,data){	
		var newHtml=Sv.tplEngine(Sv.enescape(el.Sv_tpl),data,null,true);
